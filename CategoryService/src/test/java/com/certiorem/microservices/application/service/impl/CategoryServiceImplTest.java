...
public class CategoryServiceImplTest {
	...
	public void testReadTeam() {
		...
		Category entityTeam = categoryService.readCategory(1);
	...
	public void testReadTeams() {
		...
		Mockito.when(restTemplate.exchange(CategoryConstrants.DBB_CONNECTOR_CALL_CATEGORY, HttpMethod.GET, null,
				new ParameterizedTypeReference<List<Category>>() {
				})).thenReturn(new ResponseEntity<List<Category>>(categories, HttpStatus.ACCEPTED));

		List<Category> allCategories = categoryService.readCategories();
